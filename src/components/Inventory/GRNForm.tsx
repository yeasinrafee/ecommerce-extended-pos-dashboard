...
import {
  ...
  LuBoxes, LuMapPin, LuArrowLeft, LuClipboardList, LuPackageOpen,
  LuCalendar, LuUser,
} from 'react-icons/lu';
...
export default function GRNForm() {
  ...
  const subtotal = useMemo(() =>
    (watchedItems || []).reduce((s, i) => s + (Number(i.quantityAccepted) || 0) * (Number(i.unitPrice) || 0), 0),
    [watchedItems],
  );

  const validItems = (watchedItems || []).filter(i => i.productId && Number(i.quantityReceived) >= 1);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-4 bg-white px-5 sm:px-6 py-5 rounded-xl border border-slate-200 shadow-sm">
        <button type="button" onClick={() => router.push('/inventory/grn')}
        ...
        <div className="min-w-0">
          <h1 className="text-lg sm:text-xl font-bold text-slate-900">New Goods Receive Note</h1>
          <p className="text-xs text-slate-500 mt-0.5">Fill in receipt details and items, then confirm to update stock.</p>
        </div>
      </div>
        ...
        <div className="space-y-5 min-w-0">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="sm:col-span-2 space-y-1.5">
                <label className="text-xs font-semibold text-slate-600 flex items-center gap-1.5">
                  <LuClipboardList className="h-3.5 w-3.5" /> Link Approved PO <span className="font-normal text-slate-400">(optional)</span>
                </label>
              ...
              <div className="space-y-1.5">
                ...
                {errors.receiveDate && <p className="text-red-500 text-[11px]">{errors.receiveDate.message}</p>}
              </div>

              {/* Bill Number */}
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-600 block">
                  Bill / Invoice No. <span className="font-normal text-slate-400">(optional)</span>
                </label>
          ...
          <Card className="border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div className="p-4 sm:p-5 space-y-3">
              {fields.map((field, index) => {
                ...
                return (
                  <div key={field.id} className="border border-slate-200 rounded-xl p-3 bg-white hover:bg-slate-50/40 transition-colors">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-2 items-end">
                      <div>
                        <label className="text-[10px] font-semibold text-slate-500 uppercase mb-1 block">Ordered</label>
                        <input type="number" min={0} disabled={!hasProduct}
                          {...register(`items.${index}.quantityOrdered` as const, { valueAsNumber: true })}
                          className={`w-full ${inputCls}`} />
                      </div>
                      <div>
                        <label className="text-[10px] font-semibold text-slate-500 uppercase mb-1 block">Received</label>
                      ...
                      <div>
                        <label className="text-[10px] font-semibold text-slate-500 uppercase mb-1 block">Accepted</label>
                        <input type="number" min={0} disabled={!hasProduct}
                          {...register(`items.${index}.quantityAccepted` as const, { valueAsNumber: true })}
                          className={`w-full ${inputCls}`} />
                      </div>
                      <div>
                        <label className="text-[10px] font-semibold text-slate-500 uppercase mb-1 block">Rejected</label>
                        <input type="number" min={0} disabled={!hasProduct}
                          {...register(`items.${index}.quantityRejected` as const, { valueAsNumber: true })}
                          className={`w-full ${inputCls}`} />
                      </div>
                      <div className="sm:col-span-2">
                        <label className="text-[10px] font-semibold text-slate-500 uppercase mb-1 block">Batch #</label>
                        <input type="text" placeholder="Optional" disabled={!hasProduct}
                      ...
                      <div className="sm:col-span-2">
                        <label className="text-[10px] font-semibold text-slate-500 uppercase mb-1 block">Expiry Date</label>
                        <input type="date" disabled={!hasProduct}
          ...
          <Card className="p-5 sm:p-6 border border-slate-200 shadow-sm">
            <div className="space-y-1.5">
              ...
              <textarea rows={3} placeholder="Any notes about this delivery…"
                {...register('notes')}
            ...
            <div className="space-y-2.5 text-sm">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="font-semibold text-slate-800">
                  {(watchedItems || []).reduce((s, i) => s + (Number(i.quantityReceived) || 0), 0)} units
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-slate-500">Total Accepted</span>
                <span className="font-semibold text-emerald-600">
                  {(watchedItems || []).reduce((s, i) => s + (Number(i.quantityAccepted) || 0), 0)} units
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-slate-500">Total Rejected</span>
                <span className="font-semibold text-red-500">
                  {(watchedItems || []).reduce((s, i) => s + (Number(i.quantityRejected) || 0), 0)} units
                </span>
              </div>
