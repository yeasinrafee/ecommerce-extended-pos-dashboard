...
import {
  ...
  LuBoxes, LuMapPin, LuArrowLeft, LuClipboardList,
} from 'react-icons/lu';
...
const DEFAULT_ITEM = { productId: '', quantity: 1 };

interface CustomerReturnFormProps {
  editingReturn?: CustomerReturn | null;
}

export default function CustomerReturnForm({ editingReturn }: CustomerReturnFormProps) {
  ...
  const { data: locationsRes } = useQuery({
    queryKey: ['customer-return-form', 'locations'],
    queryFn: async () => {
  ...
  const { data: productsRes } = useQuery({
    queryKey: ['customer-return-form', 'products'],
    queryFn: async () => {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      router.push('/inventory/customer-returns');
    },
    onError: (err: any) => {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-4 bg-white px-5 sm:px-6 py-5 rounded-xl border border-slate-200 shadow-sm">
        <button type="button" onClick={() => router.push('/inventory/customer-returns')}
        ...
        <div className="min-w-0">
          <h1 className="text-lg sm:text-xl font-bold text-slate-900">
            {editingReturn ? `Edit Customer Return — ${editingReturn.returnNumber}` : 'New Customer Return'}
          </h1>
          <p className="text-xs text-slate-500 mt-0.5">
            Saves as PENDING. Confirm refund from the list page to credit stock back to the location.
          </p>
        </div>
      </div>
        ...
        <div className="space-y-5 min-w-0">
          ...
          <Card className="p-5 sm:p-6 border border-slate-200 shadow-sm">
            <div className="space-y-1.5">
              ...
              <textarea rows={3} placeholder="Reason for return, condition of items, etc…"
                {...register('notes')}
          ...
          <Card className="p-4 border border-slate-200 shadow-sm">
            <div className="flex flex-col gap-2">
              ...
              <Button type="button" variant="outline"
                onClick={() => router.push('/inventory/customer-returns')}
