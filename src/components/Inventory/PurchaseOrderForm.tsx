...
import {
  ...
  LuBoxes, LuMapPin, LuArrowLeft, LuClipboardList, LuShoppingCart,
  LuCalendar, LuUser,
} from 'react-icons/lu';
...
function ProductCombobox({
  ...
  return (
    <div ref={ref} className="relative w-full min-w-0" style={{ maxWidth: '100%' }}>
      <button ref={triggerRef} type="button" disabled={disabled} onClick={handleOpen}
        ...
        {selected ? (
          <span className="min-w-0 flex-1 flex items-center gap-1.5 overflow-hidden">
            ...
            <span className="text-[10px] font-mono text-slate-400 shrink-0 bg-slate-100 px-1 py-0.5 rounded truncate max-w-[60px]">{selected.sku || '—'}</span>
          </span>
        ) : (
      ...
      {open && typeof document !== 'undefined' && ReactDOM.createPortal(
        <div ref={dropdownRef} style={style} className="bg-white border border-slate-200 rounded-xl shadow-2xl overflow-hidden">
          ...
          <div className="max-h-60 overflow-y-auto p-1" onMouseDown={e => e.preventDefault()}>
            {filtered.length === 0
...
export default function PurchaseOrderForm({ editingPO }: PurchaseOrderFormProps) {
  ...
  const { register, control, handleSubmit, setValue, reset, formState: { errors } } =
    useForm<PurchaseOrderFormValues>({
      ...
      defaultValues: {
        ...
        expectedDate: '',
        notes: '',
        items: [{ productId: '', quantity: 1, unitPrice: 0, taxPercent: 0, discountPercent: 0 }],
      },
    });
  ...
  useEffect(() => {
    if (editingPO) {
      ...
      reset({
        ...
        items: arr.length > 0
          ? arr.map(i => ({ productId: i.productId, quantity: i.quantity, unitPrice: i.unitPrice, taxPercent: i.taxPercent ?? 0, discountPercent: i.discountPercent ?? 0 }))
          : [{ productId: '', quantity: 1, unitPrice: 0, taxPercent: 0, discountPercent: 0 }],
      });
    }
  }, [editingPO, reset]);
  ...
  useEffect(() => {
    if (!editingPO && !paramsParsed && productsRes && productsRes.length > 0) {
      ...
      if (locationId || itemsStr) {
        let items = [{ productId: '', quantity: 1, unitPrice: 0, taxPercent: 0, discountPercent: 0 }];
        if (itemsStr) {
          try {
            ...
            if (Array.isArray(parsed) && parsed.length > 0) {
              items = parsed.map((i: any) => {
                ...
                const defaultPrice = prod ? (prod.Baseprice ?? prod.basePrice ?? prod.finalPrice ?? 0) : 0;
                return {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, payload }: { id: string; payload: PurchaseOrderFormValues }) =>
      apiClient.patch<ApiResponse<any>>(`/purchase-orders/update/${id}`, payload),
    onSuccess: () => {
  ...
  const onSubmit = (values: PurchaseOrderFormValues) => {
    if (isEdit && editingPO) updateMutation.mutate({ id: editingPO.id, payload: values });
  ...
  const totals = useMemo(() => {
    ...
    (watchedItems || []).forEach(item => {
      ...
      subtotal += base; discount += disc; tax += taxAmt;
    });
  ...
  const handleProductSelect = (index: number, productId: string) => {
    ...
    if (prod) {
      setValue(`items.${index}.unitPrice`, prod.Baseprice ?? prod.basePrice ?? prod.finalPrice ?? 0, { shouldValidate: true });
    }
  };
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-4 bg-white px-5 sm:px-6 py-5 rounded-xl border border-slate-200 shadow-sm">
        <button type="button" onClick={() => router.push('/inventory/purchases')}
        ...
        <div className="min-w-0">
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            {isEdit ? 'Update the draft order details and items.' : 'Saved as DRAFT — approve it to begin the GRN workflow.'}
          </p>
        </div>
      </div>
        ...
        <div className="space-y-5 min-w-0">
          ...
          <Card className="p-5 sm:p-6 border border-slate-200 shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                {errors.supplierId && <p className="text-red-500 text-[11px] flex items-center gap-1">{errors.supplierId.message}</p>}
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <select {...register('locationId')}
                  ...
                  {locationsRes?.map((l: any) => <option key={l.id} value={l.id}>{l.name} — {l.code}</option>)}
                </select>
              ...
              <div className="space-y-1.5">
                ...
                {errors.orderDate && <p className="text-red-500 text-[11px]">{errors.orderDate.message}</p>}
              </div>

              {/* Expected Date */}
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-600 flex items-center gap-1.5">
                  <LuCalendar className="h-3.5 w-3.5" /> Expected Date <span className="font-normal text-slate-400">(optional)</span>
                </label>
          ...
          <Card className="border border-slate-200 shadow-sm overflow-hidden">
            <div className="flex items-center justify-between px-5 sm:px-6 py-4 border-b border-slate-100">
              ...
              <Button type="button" variant="outline" size="sm"
                onClick={() => append({ productId: '', quantity: 1, unitPrice: 0, taxPercent: 0, discountPercent: 0 })}
            ...
            <div className="p-4 sm:p-5">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-xs min-w-[640px]">
                    <thead className="bg-slate-50 border-b border-slate-200 font-semibold text-slate-500 uppercase">
                      <tr>
                        <th className="p-3 w-[250px] max-w-[250px]">Product *</th>
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {fields.map((field, index) => {
                        ...
                        const tax = Number(watchedItems?.[index]?.taxPercent) || 0;
                        const base = q * p;
                        const lineTotal = base - base * (disc / 100) + (base - base * (disc / 100)) * (tax / 100);
              ...
              {errors.items && !Array.isArray(errors.items) && (
                <p className="mt-2 text-xs text-red-500">{errors.items.message as string}</p>
              )}
            </div>
          </Card>
          ...
          <Card className="p-5 sm:p-6 border border-slate-200 shadow-sm">
            <div className="space-y-1.5">
              ...
              <textarea rows={3} placeholder="Shipping instructions, special conditions…"
                {...register('notes')}
          ...
          <Card className="p-5 border border-slate-200 shadow-sm">
            ...
            <div className="space-y-2.5 text-sm">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="font-semibold text-red-500">৳{totals.discount.toFixed(2)}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-slate-500">Tax (+)</span>
                <span className="font-semibold text-emerald-600">৳{totals.tax.toFixed(2)}</span>
              </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="font-bold text-primary text-base">৳{totals.net.toFixed(2)}</span>
              </div>
            </div>
          </Card>
