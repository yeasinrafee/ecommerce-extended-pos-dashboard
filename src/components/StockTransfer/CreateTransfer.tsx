...
const schema = z.object({
  ...
  invoiceNumber: z
    .string()
    .regex(/^\d{12}$/, 'Invoice number must be 12 digits'),
  createDate: z
    .string()
    .min(1, 'Create date is required')
    .refine(
      (value) => !Number.isNaN(new Date(value).getTime()),
      'Create date is invalid',
    ),
  products: z
    .array(
      z.object({
...
type ProductMap = Record<
  string,
  StockTransferProductSearchResult & { image?: string }
>;
...
export default function CreateTransfer() {
  ...
  const { data: generatedInvoice, isFetching: generatingInvoice } =
    useGenerateStockTransferInvoiceNumber(true);
  ...
  const { data: searchProducts, isFetching: searchingProducts } =
    useStockTransferProductSearch(fromStoreId, nameSearchTerm);
  ...
  const selectedProductIds = React.useMemo(() => {
    return new Set(
      (watchedProducts ?? []).map((item) => item?.productId).filter(Boolean),
    );
  }, [watchedProducts]);
  ...
  const totals = React.useMemo(() => {
    ...
    const grandTotal = productRows.reduce((sum, item) => {
      ...
      const purchasePrice = Number(
        productMap[item?.productId ?? '']?.purchasePrice ?? 0,
      );
    ...
    <div className='mx-auto w-full max-w-325 p-4'>
      <div className='mb-4 flex items-center justify-between gap-3'>
        ...
        <div className='flex items-center gap-2'>
          <CustomButton
            variant='outline'
            onClick={() => router.push('/dashboard/stock-transfers/manage')}
          >
            Back
          </CustomButton>
          <CustomButton
      ...
      <form onSubmit={handleSubmit(submit)} className='space-y-5'>
        <div className='grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4'>
          ...
          <CustomSelect
            ...
            options={destinationStores.map((item) => ({
              label: item.name,
              value: item.id,
            }))}
            fieldToValue={(value: string) => value ?? ''}
          ...
          <CustomInput
            ...
            helperText={
              generatingInvoice ? 'Generating invoice number...' : undefined
            }
          />
