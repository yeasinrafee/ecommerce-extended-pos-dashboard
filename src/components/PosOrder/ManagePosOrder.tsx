      ...
      <DialogContent className='max-w-lg max-h-[90vh] overflow-y-auto rounded-2xl p-0'>
        <DialogHeader className='px-5 py-4 border-b border-slate-100'>
          ...
          <DialogDescription className='text-xs text-slate-500 mt-0.5'>
            Enter the quantity to return for each product. Stock will be
            credited back.
          </DialogDescription>
        </DialogHeader>
...
const ManagePosOrder: React.FC = () => {
  ...
  const [paymentStatusFilter, setPaymentStatusFilter] =
    React.useState<PaymentStatusFilter>('');
  ...
  /* ── customer return modal ── */
  const [returnTarget, setReturnTarget] = React.useState<PosBillSummary | null>(
    null,
  );
        ...
        renderRowActions={(item) => (
          <DropdownMenu>
            ...
            >
              ...
              <DropdownMenuItem
                onClick={() =>
                  router.push(`/dashboard/pos-order/create?editId=${item.id}`)
                }
              >
