...
const RightSection: React.FC<RightSectionProps> = ({
  ...
  const [mainImageExistingUrl, setMainImageExistingUrl] = React.useState<
    string | null
  >(null);
  const [galleryImages, setGalleryImages] = React.useState<UploadedImage[]>([]);
  const [existingGalleryUrls, setExistingGalleryUrls] = React.useState<
    string[]
  >([]);
  ...
  React.useEffect(() => {
    const hasSeeds =
      initialMainImageUrl ||
      (initialGalleryUrls?.length ?? 0) > 0 ||
      (initialCategories?.length ?? 0) > 0 ||
      (initialTags?.length ?? 0) > 0;
  ...
  const toggleCategory = (categoryId: string) => {
    setCategories((prev) => {
      ...
      if (category.parentId) {
        return prev.includes(category.parentId)
          ? [...prev, categoryId]
          : [...prev, category.parentId, categoryId];
      }
  ...
  return (
    <div className='flex w-full flex-col gap-6'>
      <div className='lg:rounded-2xl border border-dashed border-slate-300 bg-white px-6 py-6 shadow-sm'>
            ...
            {mainImageExistingUrl && !mainImage && (
              <div className='mt-2'>
                ...
                <div className='relative inline-block'>
                  <img
                    src={mainImageExistingUrl}
                    alt='Current product'
                    className='h-24 w-24 object-cover rounded-md border border-slate-200'
                  />
                  <button
                    type='button'
                ...
                <p className='text-xs text-slate-400 mt-1'>
                  Click × to replace with a new upload.
                </p>
              </div>
            )}
      ...
      <div className='lg:rounded-2xl border border-slate-200 bg-white px-4 py-4 shadow-sm'>
