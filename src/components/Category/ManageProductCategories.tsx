...
export default function ManageProductCategories() {
  ...
  const [catSearchTerm, setCatSearchTerm] = React.useState<string | undefined>(
    undefined,
  );
  const [newSubParentId, setNewSubParentId] = React.useState<
    string | undefined
  >(undefined);
  const [catDeleteTarget, setCatDeleteTarget] = React.useState<Category | null>(
    null,
  );
  ...
  const [subSearchTerm, setSubSearchTerm] = React.useState<string | undefined>(
    undefined,
  );
  const [subDeleteTarget, setSubDeleteTarget] = React.useState<Category | null>(
    null,
  );
  ...
  // ── API calls ─────────────────────────────────────────────────────
  const categoriesQuery = productApi.useParentPaginatedCategories(
    catPage,
    limit,
    catSearchTerm,
  );
  ...
  const handleSaveCategory = async (
    payload: FormData | { name: string; parentId?: string },
  ) => {
    if (catEditing) {
    ...
    () => [
      {
        ...
        cell: (row) => {
          ...
          const { initials, backgroundColor } = initialsPlaceholder(
            row.name ?? '',
          );
          return (
    ...
    () => [
      ...
      },
      {
        ...
        cell: (row) => {
          ...
          return (
            <span className='truncate block'>{parent ? parent.name : '-'}</span>
          );
        },
      },
      {
  ...
  return (
    <div>
      ...
      <div className='flex flex-col sm:flex-row items-start sm:items-center gap-3 mb-4'>
        ...
        <div className='flex gap-2 w-full sm:w-auto'>
          <CustomButton
            ...
            className='text-[11px] sm:text-xs px-2.5 sm:px-4 py-1.5 sm:py-2 flex-1 sm:flex-none'
          >
            Create Sub-Category
          </CustomButton>
          <CustomButton
      ...
      {activeTab === 'subcategories' && (
        <>
          {!allQuery.isLoading &&
          all.filter((c) => !c.parentId).length === 0 ? (
            <p className='text-slate-500 text-sm'>
              No parent categories found. Create a category first.
            </p>
          ) : allQuery.isLoading ? (
            <TableSkeleton />
          ) : allQuery.isError ? (
            ...
            <Table<Category>
              ...
              renderRowActions={(cat) => (
                <DropdownMenu>
                  ...
                  <DropdownMenuContent>
                    <DropdownMenuItem
                      onClick={() => handleEditSubcategory(cat)}
                    >
                      Edit
                    </DropdownMenuItem>
                    <DropdownMenuItem
      ...
      <CreateCategory
        ...
        initialParentId={
          catEditing
            ? ((catEditing as any).parentId ?? undefined)
            : newSubParentId
        }
