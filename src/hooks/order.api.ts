...
export type OrderStatus = "PENDING" | "SALE" | "RETURNED";
  ...
  return useMutation({
    mutationFn: async ({ ids, status }: { ids: string[]; status: string }) => {
      const requests = ids.map((id) =>
        apiClient.patch<ApiResponse<Order>>(OrderRoutes.updateStatus(id), { status })
      );
...
export const useCompleteOrder = () => {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      const response = await apiClient.patch<ApiResponse<Order>>(OrderRoutes.complete(id));
      return response.data;
    },
    onSuccess: (res) => {
      toast.success(res.message || "Order completed successfully (PENDING → SALE)");
      queryClient.invalidateQueries({ queryKey: orderKeys.all });
    },
    onError: (err: any) => {
