...
import { BlogTagRoutes } from "@/routes/blog-tag.route";
...
const fetchPaginatedTags = async (
  ...
  const response = await apiClient.get<ApiResponse<Tag[]>>(BlogTagRoutes.getAllPaginated, {
...
const fetchAllTags = async (): Promise<Tag[]> => {
  const response = await apiClient.get<ApiResponse<Tag[]>>(BlogTagRoutes.getAll);
...
export const useCreateTag = () => {
  ...
  return useMutation({
    mutationFn: async (name: string) => {
      const response = await apiClient.post<ApiResponse<Tag>>(BlogTagRoutes.create, { name });
...
export const useUpdateTag = () => {
  ...
  return useMutation({
    mutationFn: async ({ id, name }: { id: string; name: string }) => {
      const response = await apiClient.patch<ApiResponse<Tag>>(BlogTagRoutes.update(id), { name });
...
export const useDeleteTag = () => {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      const response = await apiClient.delete<ApiResponse<null>>(BlogTagRoutes.delete(id));
