...
import { ProductTagRoutes } from "@/routes/product-tag.route";
...
const fetchPaginatedTags = async (
  ...
  const response = await apiClient.get<ApiResponse<Tag[]>>(ProductTagRoutes.getAllPaginated, {
...
const fetchAllTags = async (): Promise<Tag[]> => {
  const response = await apiClient.get<ApiResponse<Tag[]>>(ProductTagRoutes.getAll);
...
export const useCreateTag = () => {
  ...
  return useMutation({
    mutationFn: async (name: string) => {
      const response = await apiClient.post<ApiResponse<Tag>>(ProductTagRoutes.create, { name });
...
export const useUpdateTag = () => {
  ...
  return useMutation({
    mutationFn: async ({ id, name }: { id: string; name: string }) => {
      const response = await apiClient.patch<ApiResponse<Tag>>(ProductTagRoutes.update(id), { name });
...
export const useDeleteTag = () => {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      const response = await apiClient.delete<ApiResponse<null>>(ProductTagRoutes.delete(id));
