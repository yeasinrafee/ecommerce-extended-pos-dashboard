...
function downloadCSV(
  ...
  const csv =
    ...
    [
      ...
      ...rows.map((r) =>
        r.map((c, i) => q(c, dateColIdx.includes(i))).join(','),
      ),
    ].join('\r\n');
...
export default function InventoryReportsPage() {
  ...
  const { data: actData, isLoading: actLoad } = useQuery({
    ...
    queryFn: async () => {
      const r = await apiClient.get<ApiResponse<ActivityRow[]>>(
        '/stocks/reports/activity',
        {
          params: {
            page: actPage,
            limit: LIMIT,
      ...
      const summary: ActivitySummary | undefined = meta
        ? {
            totalIn: meta.totalIn ?? 0,
            totalOut: meta.totalOut ?? 0,
            netChange: meta.netChange ?? 0,
            totalTransactions: meta.totalTransactions ?? meta.total ?? 0,
          }
        : undefined;
      return {
        data: r.data.data ?? ([] as ActivityRow[]),
        meta: r.data.meta ?? { page: 1, totalPages: 1, total: 0, limit: LIMIT },
        summary,
      };
    },
  });
  ...
  const { data: valData, isLoading: valLoad } = useQuery({
    ...
    queryFn: async () => {
      const d = (
        await apiClient.get<ApiResponse<any>>('/stocks/reports/current', {
          params: { locationId: location || undefined },
        })
      ).data.data;
      return {
        summary: d?.summary as ValuationSummary | null,
        locationBreakdown: (d?.locationBreakdown ?? []) as LocationBreakdown[],
        productBreakdown: (d?.productBreakdown ?? []) as ProductBreakdown[],
      };
    },
      ...
      const r = await apiClient.get<ApiResponse<any>>(
        '/stocks/reports/transfers',
        {
          params: {
            page: trfPage,
            limit: LIMIT,
            sourceLocationId: location || undefined,
            startDate: start || undefined,
            endDate: end || undefined,
          },
        },
  ...
  const exportPDF = async () => {
    ...
    y += 5;
    if (start || end) {
      doc.text(`Period: ${start || '—'} to ${end || '—'}`, 14, y);
      y += 5;
    }
    if (location) {
      const ln = locList.find((l: any) => l.id === location)?.name ?? location;
      doc.text(`Location: ${ln}`, 14, y);
      y += 5;
    }
    ...
    if (tab === 'activity') {
      ...
      autoTable(doc, {
        ...
        body: rows.map((r: ActivityRow) => [
          ...
          r.location.name,
          r.previousQty,
          (r.quantityChanged > 0 ? '+' : '') + r.quantityChanged,
          r.currentQty,
          fmt(r.unitCost),
          fmt(r.totalCostImpact),
          r.referenceType,
          r.performedBy,
          r.notes,
        ]),
      ...
      if (s) {
        autoTable(doc, {
          ...
          head: [['Metric', 'Value']],
          body: [
            ['Total Unique Products', s.totalUniqueProducts],
            ['Total Physical Qty', s.totalPhysicalQuantity],
            ['Total Reserved Qty', s.totalReservedQuantity],
            ['Cost Valuation (BDT)', fmt(s.totalValueCost)],
            ['Retail Valuation (BDT)', fmt(s.totalValueRetail)],
          ],
          tableWidth: 120,
        });
      ...
      autoTable(doc, {
        ...
        body: (valData?.locationBreakdown ?? []).map((l) => [
          l.name,
          l.code,
          l.distinctProducts,
          l.quantity,
      ...
      autoTable(doc, {
        ...
        body: (valData?.productBreakdown ?? []).map((p) => [
          p.productName,
          p.sku ?? '—',
          p.totalQty,
          p.totalReserved,
      ...
      autoTable(doc, {
        ...tStyle,
        head: [['Date', 'Transfer #', 'From', 'To', 'Status', 'Created By']],
        body: (trfData?.data ?? []).map((t: any) => [
  ...
  const exportExcel = () => {
    ...
    if (tab === 'activity') {
      downloadCSV(
        ...
        (actData?.data ?? []).map((r: ActivityRow) => [
          r.dateFormatted,
          r.timeFormatted,
          r.movementLabel,
          r.direction,
      ...
      downloadCSV(
        ...
        (valData?.locationBreakdown ?? []).map((l) => [
          l.name,
          l.code,
          l.distinctProducts,
          l.quantity,
      ...
      downloadCSV(
        ...
        (trfData?.data ?? []).map((t: any) => [
          fd(t.createdAt),
          t.transferNumber ?? t.id,
          t.sourceLocation?.name,
          t.destinationLocation?.name,
          t.status,
          t.transferDate ? fd(t.transferDate) : '—',
          t.receivedDate ? fd(t.receivedDate) : '—',
          t.creator?.email,
        ]),
        [0, 5, 6],
      );
  ...
  const Empty = ({ text }: { text: string }) => (
    <div className='p-16 text-center text-slate-400 text-sm'>{text}</div>
  );
  ...
  const Sel = ({
    ...
    <div>
      ...
      <select
        ...
        className='w-full bg-slate-50 border border-slate-200 text-slate-800 text-sm h-9 rounded-lg px-3 outline-none focus:ring-1 focus:ring-slate-300'
      >
        {opts.map((o) => (
  ...
  const locOpts = [
    ...
    ...locList.map((l: any) => ({ value: l.id, label: l.name })),
  ];
  const movTypeOpts = [
      ...
      <div className='flex flex-col gap-3 md:flex-row md:items-center md:justify-between bg-white px-6 py-5 rounded-2xl border border-slate-100 shadow-sm'>
        <div>
          ...
          <p className='text-xs text-slate-500 mt-0.5'>
            Detailed activity log, valuation, movements, transfers, damages,
            adjustments and low-stock.
          </p>
        </div>
        <div className='flex gap-2'>
          <Button
            ...
            size='sm'
            onClick={exportExcel}
            className='text-xs gap-1.5 rounded-xl border-slate-200'
          >
            Excel
          </Button>
          <Button
          ...
          {tab === 'activity' && (
            <div className='col-span-2'>
              ...
              <input
                ...
                placeholder='Product name, SKU, notes…'
                className='w-full bg-slate-50 border border-slate-200 text-sm h-9 rounded-lg px-3 outline-none focus:ring-1 focus:ring-slate-300'
              />
            </div>
          )}
        </div>
        ...
        <TabsContent value='activity'>
          <Card className='border-slate-100 shadow-sm overflow-hidden'>
                ...
                {[
                  {
                    ...
                    val: actData.summary.totalTransactions,
                    color: 'text-slate-800',
                  },
                  {
                    label: 'Total Stock IN',
                    val: `+${actData.summary.totalIn} qty`,
                    color: 'text-emerald-700',
                  },
                  {
                    label: 'Total Stock OUT',
                    val: `-${actData.summary.totalOut} qty`,
                    color: 'text-rose-700',
                  },
                  {
                    label: 'Net Change',
                    val:
                      (actData.summary.netChange >= 0 ? '+' : '') +
                      actData.summary.netChange +
                      ' qty',
                    color:
                      actData.summary.netChange >= 0
                        ? 'text-emerald-700'
            ...
            {actLoad ? (
              ...
              <Empty text='No activity found for the selected filters.' />
            ) : (
                  ...
                  <thead className='bg-slate-50 border-b border-slate-200'>
                    <tr>
                      {[
                        ...
                        <th
                          key={h}
                          className='px-3 py-2.5 text-left font-semibold text-slate-600 whitespace-nowrap border-b border-slate-200'
                        >
                          {h}
                        </th>
                      ))}
                    ...
                    {actData.data.map((r: ActivityRow, i: number) => (
                      ...
                      >
                        ...
                        <td className='px-3 py-2'>
                          <span
                            className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold border ${DIRECTION_COLOR[r.direction]}`}
                          >
                            {r.direction === 'IN' ? '▲ IN' : '▼ OUT'}
                          </span>
                        </td>
                        <td
                          className='px-3 py-2 max-w-[160px] truncate font-medium text-slate-800'
                        ...
                        <td
                          className={`px-3 py-2 text-center font-bold ${r.quantityChanged > 0 ? 'text-emerald-700' : 'text-rose-700'}`}
                        >
                          {r.quantityChanged > 0
                            ? `+${r.quantityChanged}`
                            : r.quantityChanged}
                        ...
                        <td
                          className={`px-3 py-2 text-right font-semibold ${r.direction === 'OUT' ? 'text-rose-700' : 'text-emerald-700'}`}
                        >
                          {fmt(r.totalCostImpact)}
                        </td>
                        <td className='px-3 py-2 whitespace-nowrap text-slate-500'>
                        ...
                        <td
                          className='px-3 py-2 max-w-[180px] truncate text-slate-400'
                          title={r.notes}
                        >
                          {r.notes}
                        </td>
          ...
          <div className='space-y-4'>
            {valLoad ? (
              ...
              <>
                {valData?.summary && (
                  <div className='grid grid-cols-2 md:grid-cols-5 gap-px bg-slate-100 rounded-xl overflow-hidden border border-slate-100'>
                    {[
                      {
                        l: 'Unique Products',
                        v: valData.summary.totalUniqueProducts,
                      },
                      {
                        l: 'Physical Qty',
                        v: valData.summary.totalPhysicalQuantity,
                      },
                      {
                        l: 'Reserved Qty',
                        v: valData.summary.totalReservedQuantity,
                      },
                      {
                        l: 'Cost Value (BDT)',
                        v: fmt(valData.summary.totalValueCost),
                      },
                      {
                        l: 'Retail Value (BDT)',
                        v: fmt(valData.summary.totalValueRetail),
                      },
                    ].map(({ l, v }) => (
                ...
                <Card className='border-slate-100 shadow-sm overflow-hidden'>
                  <div className='px-4 py-3 border-b border-slate-100 bg-slate-50 text-xs font-semibold text-slate-600 uppercase tracking-wide'>
                    Location Breakdown
                  </div>
                  <div className='overflow-x-auto'>
                    <table className='w-full text-xs border-collapse'>
                      <thead className='bg-slate-50 border-b'>
                        <tr>
                          <th className='px-4 py-2.5 text-left font-semibold text-slate-600 min-w-[140px]'>
                            Location
                          </th>
                          <th className='px-4 py-2.5 text-left font-semibold text-slate-600 whitespace-nowrap'>
                            Code
                          </th>
                          <th className='px-4 py-2.5 text-right font-semibold text-slate-600 whitespace-nowrap'>
                            Distinct Products
                          </th>
                          <th className='px-4 py-2.5 text-right font-semibold text-slate-600 whitespace-nowrap'>
                            Stock Qty
                      ...
                      <tbody>
                        {(valData?.locationBreakdown ?? []).map((l, i) => (
                          ...
                          >
                            <td className='px-4 py-2 font-medium text-slate-800 min-w-[140px] wrap-break-word'>
                              {l.name}
                            </td>
                ...
                <Card className='border-slate-100 shadow-sm overflow-hidden'>
                  ...
                  <div className='overflow-x-auto'>
                    <table className='w-full text-xs border-collapse'>
                      <thead className='bg-slate-50 border-b'>
                        <tr>
                          <th className='px-4 py-2.5 text-left font-semibold text-slate-600 min-w-[160px]'>
                            Product
                          </th>
                          <th className='px-4 py-2.5 text-left font-semibold text-slate-600 whitespace-nowrap'>
                            SKU
                      ...
                      <tbody>
                        {(valData?.productBreakdown ?? []).map((p, i) => (
                          ...
                          >
                            <td className='px-4 py-2 font-medium text-slate-800 min-w-[160px] wrap-break-word'>
                              {p.productName}
                            </td>
                            <td className='px-4 py-2 text-slate-500 whitespace-nowrap'>
                              {p.sku ?? '—'}
                  ...
                  <tbody>
                    {(movData.data ?? []).map((m: any, i: number) => (
                      ...
                      >
                        ...
                        <td className='px-3 py-2 font-medium text-slate-800 max-w-[150px] truncate'>
                          {m.product?.name}
                        </td>
                        ...
                        <td
                          className={`px-3 py-2 text-center font-bold ${m.quantityChanged > 0 ? 'text-emerald-700' : 'text-rose-700'}`}
                        >
                          {m.quantityChanged > 0
                            ? `+${m.quantityChanged}`
                            : m.quantityChanged}
                  ...
                  <tbody>
                    {(trfData.data ?? []).map((t: any, i: number) => (
                      ...
                      >
                        ...
                        <td className='px-3 py-2 text-slate-500'>
                          {t.receivedDate
                            ? new Date(t.receivedDate).toLocaleDateString()
                            : '—'}
                        </td>
                        <td className='px-3 py-2 text-slate-500'>
                          {t.creator?.email}
        ...
        <TabsContent value='damages'>
          <Card className='border-slate-100 shadow-sm overflow-hidden'>
            {dmgLoad ? (
              ...
              <Empty text='No damage records found.' />
            ) : (
              <div className='overflow-x-auto'>
                <table className='w-full text-xs border-collapse'>
                  <thead className='bg-slate-50 border-b'>
                    <tr>
                      ...
                      <th className='px-3 py-2.5 text-left font-semibold text-slate-600 min-w-[120px]'>
                        Location
                      </th>
                      <th className='px-3 py-2.5 text-left font-semibold text-slate-600 whitespace-nowrap'>
                        Status
                      ...
                      <th className='px-3 py-2.5 text-right font-semibold text-slate-600 whitespace-nowrap'>
                        Total Qty
                      </th>
                      <th className='px-3 py-2.5 text-right font-semibold text-slate-600 whitespace-nowrap'>
                        Total Loss (BDT)
                    ...
                    {(dmgData.data ?? []).map((d: any, i: number) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className='px-3 py-2 text-slate-500 min-w-[160px]'>
                            {(d.items ?? [])
        ...
        <TabsContent value='adjustments'>
          <Card className='border-slate-100 shadow-sm overflow-hidden'>
            {adjLoad ? (
              ...
              <Empty text='No adjustment records found.' />
            ) : (
              <div className='overflow-x-auto'>
                <table className='w-full text-xs border-collapse'>
                  <thead className='bg-slate-50 border-b'>
                    ...
                    {(adjData.data ?? []).map((a: any, i: number) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className='px-3 py-2 text-center font-semibold text-rose-700'>
                            {a.totalRemoved ?? 0}
                          </td>
                          <td
                            className={`px-3 py-2 text-center font-bold ${net >= 0 ? 'text-emerald-700' : 'text-rose-700'}`}
                          >
                            {net >= 0 ? `+${net}` : net}
                          </td>
                          <td className='px-3 py-2 text-center text-slate-500'>
        ...
        <TabsContent value='lowstock'>
          <Card className='border-slate-100 shadow-sm overflow-hidden'>
            {lstLoad ? (
              ...
              <Empty text='No low-stock products found.' />
            ) : (
              <div className='overflow-x-auto'>
                <table className='w-full text-xs border-collapse'>
                  <thead className='bg-slate-50 border-b'>
                  ...
                  <tbody>
                    {(lstData.data ?? []).map((l: any, i: number) => {
                      const deficit = Math.max(
                        0,
                        l.minimumQuantity - l.currentQuantity,
                      );
                      return (
                        <tr
                          key={`${l.productId}-${l.locationId}`}
                          className={
                            i % 2 === 0 ? 'bg-white' : 'bg-slate-50/60'
                          }
                        >
