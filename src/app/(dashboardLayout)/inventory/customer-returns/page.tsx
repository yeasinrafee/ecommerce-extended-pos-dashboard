...
const totalQty = (items: ReturnItem[]) =>
...
export default function CustomerReturnsPage() {
  ...
  const [detailsReturn, setDetailsReturn] = useState<CustomerReturn | null>(
    null,
  );
  ...
  const { data: returnsRes, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const r = await apiClient.get<ApiResponse<any>>(
        '/customer-returns/get-all-paginated',
        {
          params: {
            page,
            limit,
  ...
  const fetchById = async (id: string): Promise<CustomerReturn> => {
    const r = await apiClient.get<ApiResponse<CustomerReturn>>(
      `/customer-returns/get/${id}`,
    );
  ...
  const refundMutation = useMutation({
    mutationFn: (id: string) =>
      apiClient.patch<ApiResponse<any>>(`/customer-returns/refund/${id}`),
    onSuccess: async (res) => {
  ...
  const cancelMutation = useMutation({
    mutationFn: (id: string) =>
      apiClient.patch<ApiResponse<any>>(`/customer-returns/cancel/${id}`),
    onSuccess: () => {
      ...
      <div className='flex flex-col gap-4 md:flex-row md:items-center md:justify-between bg-white p-6 rounded-xl border border-gray-100 shadow-sm'>
        <div>
          ...
          <p className='text-xs text-slate-500 mt-0.5'>
            Log returned items, then confirm refund to credit stock back to a
            location.
          </p>
        </div>
        <Button
        ...
        {isLoading ? (
          ...
          <div className='p-12 text-center text-slate-500'>
            ...
            <p className='text-xs text-slate-400 mt-1'>
              Create a return to credit stock back to a location.
            </p>
          </div>
        ) : (
              ...
              <tbody className='divide-y divide-slate-200 text-sm text-slate-700'>
                {itemsList.map((ret) => (
                  ...
                  >
                    ...
                    <td className='p-4 hidden md:table-cell text-slate-500 max-w-[180px] truncate'>
                      {ret.notes || '—'}
                    </td>
                          ...
                          {ret.status === 'PENDING' && (
                            <>
                              ...
                              <DropdownMenuItem
                                onClick={() =>
                                  router.push(
                                    `/inventory/customer-returns/edit/${ret.id}`,
                                  )
                                }
        ...
        {detailsReturn && (
          <DialogContent className='w-full max-w-[calc(100%-1rem)] sm:max-w-xl max-h-[90vh] overflow-y-auto rounded-xl sm:rounded-2xl p-0'>
            ...
            <div className='sticky top-0 z-10 bg-white border-b border-slate-100 px-5 py-4 flex items-center justify-between gap-3'>
              ...
              <Badge
                className={`font-semibold py-0.5 px-2.5 text-[10px] uppercase rounded-full border-0 ${statusColors[detailsReturn.status]}`}
              >
            ...
            <div className='px-5 py-4 space-y-4'>
              ...
              {(detailsReturn.items?.length ?? 0) > 0 ? (
                ...
                <div className='text-center py-6 text-xs text-slate-400 bg-slate-50 rounded-xl border border-slate-200'>
                  Item details not available in list view — fetched on
                  individual load.
                </div>
              )}
                ...
                {detailsReturn.status === 'PENDING' && (
                  <Button
                    ...
                    onClick={() => {
                      ...
                      setRefundTarget(detailsReturn);
                    }}
                    className='bg-green-600 hover:bg-green-700 text-white text-xs h-8 px-4'
                  >
                    <LuCircleCheck className='h-3.5 w-3.5 mr-1.5' /> Confirm
                    Refund
                  </Button>
                )}
                <Button
