...
export default function InventoryDashboardPage() {
  ...
  return (
    <div className='p-0 lg:p-6 space-y-6'>
      {/* ── Header ──────────────────────────────────────────────────────────── */}
      <div className='bg-white p-3 sm:p-6 rounded-none lg:rounded-2xl border border-gray-100 shadow-sm flex flex-col md:flex-row md:items-center md:justify-between gap-3'>
        <div>
      ...
      <div className='grid gap-3 sm:gap-4 grid-cols-2 sm:grid-cols-2 lg:grid-cols-4'>
        {kpiRow1.map(({ label, value, sub, icon: Icon, color }) => (
          <Card
            key={label}
            className='p-3 sm:p-4 border-slate-100 shadow-sm flex items-start justify-between gap-2 sm:gap-3'
          >
            ...
            <div
              className={`p-2 sm:p-2.5 rounded-lg sm:rounded-xl shrink-0 hidden sm:flex ${color}`}
            >
      ...
      <div className='grid gap-3 sm:gap-4 grid-cols-2 sm:grid-cols-2 lg:grid-cols-4'>
        {kpiRow2.map(
          ({ label, value, sub, icon: Icon, color, alert, href }) => (
            <Card
              key={label}
              className={`p-3 sm:p-4 border-slate-100 shadow-sm flex flex-col gap-3 ${alert ? 'border-red-200 bg-red-50/30' : ''}`}
            >
              <div className='flex items-start justify-between gap-2 sm:gap-3'>
                <div className='space-y-0.5 min-w-0 flex-1'>
                  ...
                  <p
                    className={`text-sm sm:text-xl font-bold ${alert ? 'text-red-600' : 'text-slate-900'}`}
                  >
                    {value}
                  </p>
                  <p className='text-[10px] text-slate-400 truncate'>{sub}</p>
      ...
      {/* ── Quick Actions ───────────────────────────────────────────────────── */}
      <Card className='p-3 sm:p-5 border-slate-100 shadow-sm rounded-none lg:rounded-2xl'>
        <h2 className='text-xs font-bold text-slate-500 uppercase tracking-wider mb-3 sm:mb-4'>
          Quick Actions
        </h2>
        <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-2 sm:gap-3'>
          {[
            ...
            <Link
              key={href}
              href={href}
              className={`flex flex-col items-center justify-center p-2.5 sm:p-3 bg-slate-50 border border-slate-200 rounded-lg sm:rounded-xl transition-all group text-slate-600 ${color}`}
            >
      ...
      <div className='grid gap-3 sm:gap-6 lg:grid-cols-2'>
        {/* Recent Movements */}
        <Card className='p-3 sm:p-5 border-slate-100 shadow-sm flex flex-col rounded-none lg:rounded-2xl'>
          ...
          {movements.length === 0 ? (
            ...
            <div className='space-y-0 divide-y divide-slate-50'>
              {movements.map((mv) => (
                <div
                  key={mv.id}
                  className='py-2.5 sm:py-3 flex items-center justify-between gap-2 sm:gap-3 first:pt-0 last:pb-0'
                >
                  <div className='flex items-start gap-2 sm:gap-3 min-w-0 flex-1'>
                    ...
                    <div className='min-w-0'>
                      <p className='text-xs sm:text-sm font-semibold text-slate-900 truncate max-w-32 sm:max-w-none'>
                        {mv.product?.name}
                      </p>
                      <p className='text-[10px] text-slate-400 truncate max-w-32 sm:max-w-none'>
                        {mv.location?.name}
                      </p>
                    </div>
                  </div>
                  <div className='flex flex-col items-end gap-1 shrink-0'>
                    ...
                    <span
                      className={`text-xs sm:text-sm font-bold ${mv.quantityChanged > 0 ? 'text-green-600' : 'text-red-500'}`}
                    >
              ...
              {lowStockAlerts.map((alert) => {
                ...
                return (
                  <div
                    ...
                    className='py-2.5 sm:py-3 flex items-start justify-between gap-2 sm:gap-3 first:pt-0 last:pb-0'
                  >
                    <div className='min-w-0 flex-1'>
                      <p className='text-xs sm:text-sm font-semibold text-slate-900 truncate max-w-36 sm:max-w-none'>
                        {alert.productName}
                      </p>
                      <div className='flex items-center gap-1 mt-0.5'>
                        <LuMapPin className='h-2.5 w-2.5 text-slate-400 shrink-0' />
                        <p className='text-[10px] text-slate-400 truncate max-w-36 sm:max-w-none'>
                          {alert.locationName}
                        </p>
                        {alert.sku && (
          ...
          {lowStockAlerts.length > 0 && (
            <div className='pt-4 mt-3 border-t border-slate-100'>
              <Link
                ...
                className='flex items-center justify-center gap-1.5 w-full py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-[11px] sm:text-xs font-semibold rounded-lg sm:rounded-xl transition-colors'
              >
                <LuShoppingCart className='h-3.5 w-3.5 shrink-0' />
