...
import {
  LuSearch, LuPlus, LuEye, LuRefreshCw, LuArrowRight, LuTruck, LuPackage, LuX,
} from 'react-icons/lu';
...
export default function StockTransfersPage() {
  ...
  const shipMutation = useMutation({
    mutationFn: (id: string) => apiClient.patch<ApiResponse<any>>(`/stock-transfers/ship/${id}`),
    onSuccess: () => {
    ...
    onError: (e: any) => { toast.error(e?.response?.data?.message || 'Failed to ship'); setShipConfirmId(null); },
  });

  const cancelMutation = useMutation({
    mutationFn: (id: string) => apiClient.patch<ApiResponse<any>>(`/stock-transfers/cancel/${id}`),
    onSuccess: () => {
    ...
    onError: (e: any) => { toast.error(e?.response?.data?.message || 'Failed to cancel'); setCancelConfirmId(null); },
  });

  // ─── Handlers ─────────────────────────────────────────────────────────────────
  const handleViewClick = async (tr: StockTransfer) => {
    setDetailsLoading(true);
    try { setDetailsTransfer(await fetchById(tr.id)); }
    catch { setDetailsTransfer({ ...tr, items: tr.items || [] }); }
      ...
      <Card className="p-4 border-slate-100 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="relative">
            ...
            <input type="text" placeholder="Search transfer #…" value={searchTerm}
        ...
        {isLoading ? (
          ...
          <div className="overflow-x-auto">
              ...
              <tbody className="divide-y divide-slate-200 text-sm text-slate-700">
                {itemsList.map(tr => (
                  <tr key={tr.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="p-4">
                      <p className="font-semibold text-slate-900">{tr.sourceLocation?.name}</p>
                      <p className="text-[10px] font-mono text-slate-400">{tr.sourceLocation?.code}</p>
                    </td>
                    <td className="p-4 text-slate-300"><LuArrowRight className="h-4 w-4" /></td>
                    <td className="p-4">
                      <p className="font-semibold text-slate-900">{tr.destinationLocation?.name}</p>
                      <p className="text-[10px] font-mono text-slate-400">{tr.destinationLocation?.code}</p>
                    </td>
                    <td className="p-4 text-center font-semibold text-slate-600">
                      {tr.items?.length ?? '—'}
                    </td>
                    <td className="p-4 text-center">
                      <Badge className={`font-semibold py-0.5 px-2 text-[10px] uppercase rounded-full border-0 ${statusColors[tr.status] || 'bg-slate-100 text-slate-600'}`}>
                    ...
                    <td className="p-4 text-center">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-48 rounded-xl border-slate-100 shadow-lg">
                          <DropdownMenuItem onClick={() => handleViewClick(tr)} disabled={detailsLoading}
                          ...
                          {tr.status === 'DRAFT' && (
                            <>
                              ...
                              <DropdownMenuSeparator />
                              <DropdownMenuItem onClick={() => setCancelConfirmId(tr.id)}
                          ...
                          {tr.status === 'IN_TRANSIT' && (
                            <>
                              <DropdownMenuSeparator />
                              <DropdownMenuItem onClick={() => router.push(`/inventory/transfers/receive/${tr.id}`)}
            ...
            <div className="p-4 border-t border-slate-100">
              <PaginationControl
                currentPage={meta.page} totalPages={meta.totalPages} onPageChange={setPage}
                totalItems={meta.total} itemsPerPage={limit}
        ...
        {detailsTransfer && (
          <DialogContent className="w-full max-w-[calc(100%-1rem)] sm:max-w-2xl lg:max-w-3xl max-h-[90vh] overflow-y-auto rounded-xl sm:rounded-2xl p-0">
            ...
            <div className="sticky top-0 z-10 bg-white border-b border-slate-100 px-5 py-4 flex items-center justify-between gap-3">
              <div>
                <h2 className="text-sm font-bold text-slate-900">{detailsTransfer.transferNumber}</h2>
                <p className="text-[11px] text-slate-400 mt-0.5">{new Date(detailsTransfer.createdAt).toLocaleString()}</p>
              </div>
              <Badge className={`font-semibold py-0.5 px-2.5 text-[10px] uppercase rounded-full border-0 ${statusColors[detailsTransfer.status]}`}>
            ...
            <div className="px-5 py-4 space-y-4">
              <div className="grid grid-cols-3 gap-3 text-xs">
                <div className="bg-slate-50 rounded-xl p-3">
                  <p className="text-slate-400 font-medium uppercase tracking-wide text-[10px] mb-0.5">From</p>
                  <p className="font-semibold text-slate-900">{detailsTransfer.sourceLocation?.name}</p>
                  <p className="text-[10px] font-mono text-slate-400">{detailsTransfer.sourceLocation?.code}</p>
                </div>
                <div className="flex items-center justify-center">
                  <LuArrowRight className="h-5 w-5 text-slate-400" />
                </div>
                <div className="bg-slate-50 rounded-xl p-3">
                  <p className="text-slate-400 font-medium uppercase tracking-wide text-[10px] mb-0.5">To</p>
                  <p className="font-semibold text-slate-900">{detailsTransfer.destinationLocation?.name}</p>
                  <p className="text-[10px] font-mono text-slate-400">{detailsTransfer.destinationLocation?.code}</p>
                </div>
              </div>
              ...
              {detailsTransfer.notes && (
                <div className="bg-slate-50 rounded-xl p-3 text-xs">
                  ...
                  <p className="text-slate-700 whitespace-pre-wrap">{detailsTransfer.notes}</p>
                </div>
              )}
              <div className="flex justify-end gap-2 pt-1">
                {detailsTransfer.status === 'DRAFT' && (
                  <Button size="sm" onClick={() => { setDetailsTransfer(null); setShipConfirmId(detailsTransfer.id); }}
                ...
                {detailsTransfer.status === 'IN_TRANSIT' && (
                  <Button size="sm" onClick={() => { setDetailsTransfer(null); router.push(`/inventory/transfers/receive/${detailsTransfer.id}`); }}
                    className="bg-green-600 hover:bg-green-700 text-white text-xs h-8 px-4">
                    <LuPackage className="h-3.5 w-3.5 mr-1.5" /> Receive
                  </Button>
                )}
                <Button variant="outline" size="sm" onClick={() => setDetailsTransfer(null)} className="text-xs h-8">Close</Button>
              </div>
            </div>
          </DialogContent>
        )}
