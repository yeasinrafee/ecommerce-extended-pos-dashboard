...
import { LuSearch, LuPlus, LuEye, LuRefreshCw, LuCircleCheck, LuCircleX } from 'react-icons/lu';
...
import { type Damage, reasonLabels, reasonBadgeColors } from '@/components/Inventory/DamageForm';
...
export default function DamageInventoryPage() {
  ...
  const { data: damagesRes, isLoading } = useQuery({
    queryKey: ['damages', 'list', page, limit, searchTerm, selectedLocation, selectedStatus],
    queryFn: async () => {
            ...
            <table className="w-full border-collapse text-left">
              ...
              <tbody className="divide-y divide-slate-200 text-sm text-slate-700">
                {itemsList.map((dmg: Damage) => {
                  const totalQty = dmg.items?.reduce((s, i) => s + (i.quantity || 0), 0) ?? 0;
                  const reasons = [...new Set(dmg.items?.map(i => i.reason).filter(Boolean))];
                  return (
                    <tr key={dmg.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        {dmg.items && dmg.items.length > 0 ? (
                          <div className="max-w-[200px] truncate">
                            <p className="font-semibold text-slate-900 truncate"
                              title={dmg.items.map(i => i.product?.name).filter(Boolean).join(', ')}>
                              {dmg.items.map(i => i.product?.name).filter(Boolean).join(', ')}
                            </p>
                            {dmg.items.length === 1 && (
                              <p className="text-[10px] font-mono text-slate-400">{dmg.items[0].product?.sku}</p>
                            )}
                          </div>
                        ) : (
                      ...
                      <td className="p-4 text-center">
                        {reasons.length > 0 ? (
                          <div className="flex flex-wrap justify-center gap-1">
                            ...
                            {reasons.length > 2 && (
                              <span className="text-[10px] text-slate-400">+{reasons.length - 2}</span>
                            )}
                          </div>
                        ) : (
                          <span className="text-slate-400">—</span>
                      ...
                      <td className="p-4 hidden sm:table-cell text-slate-500 text-center">
                        {new Date(dmg.damageDate || dmg.createdAt).toLocaleDateString()}
                      </td>
                      <td className="p-4 text-center">
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="w-48 rounded-xl border-slate-100 shadow-lg">
                            ...
                            {dmg.status === 'DRAFT' && (
                              <>
                                ...
                                <DropdownMenuItem
                                  onClick={() => router.push(`/inventory/damages/edit/${dmg.id}`)}
            ...
            <div className="sticky top-0 z-10 bg-white border-b border-slate-100 px-5 py-4 flex items-center justify-between gap-3">
              <div>
                ...
                <p className="text-[11px] text-slate-400 mt-0.5">
                  {new Date(detailsDamage.damageDate || detailsDamage.createdAt).toLocaleString()}
                </p>
              </div>
            ...
            <div className="px-5 py-4 space-y-4">
              <div className="grid grid-cols-2 gap-3 text-xs">
                ...
                <div className="bg-slate-50 rounded-xl p-3">
                  ...
                  <p className="font-semibold text-slate-900">
                    {new Date(detailsDamage.damageDate || detailsDamage.createdAt).toLocaleDateString()}
                  </p>
                </div>
              </div>
              ...
              <div className="flex justify-end gap-2 pt-1">
                {detailsDamage.status === 'DRAFT' && (
                  <Button size="sm"
                    onClick={() => { setDetailsDamage(null); setCompleteTarget(detailsDamage); }}
                    className="bg-green-600 hover:bg-green-700 text-white text-xs h-8 px-4">
                    <LuCircleCheck className="h-3.5 w-3.5 mr-1.5" /> Complete Write-off
                  </Button>
                )}
                <Button variant="outline" size="sm" onClick={() => setDetailsDamage(null)} className="text-xs h-8">
                  Close
                </Button>
              </div>
            </div>
