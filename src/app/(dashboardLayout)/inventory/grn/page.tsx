...
export default function GRNPage() {
  ...
  const { data: grnRes, isLoading: isLoadingGRNs } = useQuery({
    ...
    queryFn: async () => {
      const r = await apiClient.get<ApiResponse<any>>(
        '/goods-receives/get-all-paginated',
        {
          params: {
            page,
            limit,
  ...
  const handleViewGRN = async (grn: GoodsReceive) => {
    ...
    try {
      const r = await apiClient.get<ApiResponse<GoodsReceive>>(
        `/goods-receives/get/${grn.id}`,
      );
      ...
      <div className='flex flex-col gap-4 md:flex-row md:items-center md:justify-between bg-white p-6 rounded-xl border border-gray-100 shadow-sm'>
        <div>
          ...
          <p className='text-xs text-slate-500 mt-0.5'>
            Record incoming shipments, verify quantities, then confirm to update
            stock.
          </p>
        </div>
        <Button
        ...
        {isLoadingGRNs ? (
          ...
          <div className='p-12 text-center text-slate-500'>
            ...
            <p className='text-xs text-slate-400 mt-1'>
              Create a GRN to check-in stock items.
            </p>
          </div>
        ) : (
                  ...
                  >
                    ...
                    <td className='p-4 hidden md:table-cell text-slate-500'>
                      {grn.purchaseOrder?.poNumber || (
                        <span className='text-slate-300'>Direct In</span>
                      )}
                    </td>
                    ...
                    <td className='p-4 text-center'>
                      <Badge
                        className={`font-semibold py-0.5 px-2 text-[10px] uppercase rounded-full border-0 ${statusColors[grn.status] || 'bg-slate-100 text-slate-700'}`}
                      >
                        {grn.status}
                      </Badge>
                    </td>
                    <td className='p-4 text-center'>
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent
                          align='end'
                          className='w-44 rounded-xl border-slate-100 shadow-lg'
                        >
                          <DropdownMenuItem
        ...
        <DialogContent className='w-full max-w-[calc(100%-1rem)] sm:max-w-2xl max-h-[90vh] overflow-y-auto rounded-xl sm:rounded-2xl p-0'>
          ...
          <div className='sticky top-0 z-10 bg-white border-b border-slate-100 px-5 py-4'>
            <div className='flex items-center justify-between gap-3'>
              ...
              {detailsGRN && (
                <Badge
                  className={`font-semibold py-0.5 px-2.5 text-[10px] uppercase rounded-full border-0 shrink-0 ${statusColors[detailsGRN.status]}`}
                >
                  {detailsGRN.status}
                </Badge>
              )}
            ...
            <div className='px-5 py-4 space-y-4'>
              ...
              <div className='grid grid-cols-2 gap-3 text-xs'>
                ...
                <div className='bg-slate-50 rounded-xl p-3 space-y-0.5'>
                  ...
                  <p className='font-semibold text-slate-900'>
                    {new Date(detailsGRN.receiveDate).toLocaleDateString()}
                  </p>
                </div>
                <div className='bg-slate-50 rounded-xl p-3 space-y-0.5'>
                  <p className='text-slate-400 font-medium uppercase tracking-wide text-[10px]'>
                  ...
                  <p className='font-semibold text-slate-900'>
                    {detailsGRN.purchaseOrder?.poNumber || 'Direct In'}
                  </p>
                </div>
                ...
                {detailsGRN.billAmount != null && (
                  <div className='bg-slate-50 rounded-xl p-3 space-y-0.5'>
                    ...
                    <p className='font-semibold text-slate-900'>
                      ৳{Number(detailsGRN.billAmount).toFixed(2)}
                    </p>
                  </div>
                )}
              </div>
